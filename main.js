...
const projects = [
  ...
  },
  {
    ...
    "tasks": [
      ...
      {
        "id": "pb-5",
        "title": "Place parts order by Jun 26 (4-wk lead time → parts arrive Jul 24)",
        "tags": [
          "ops"
        ],
        "due": "Jun 26",
...
const spareParts = [
  {
    "id": "sp-formic-101180",
    "poNumber": "101180",
    "customer": "Formic Robots LLC",
    "contact": "Clarence Young",
    "address": "1 Red Devil Road, Hannibal, MO 63401",
  ...
  {
    "id": "sp-crg-p0200-00542",
    "poNumber": "P0200-00542",
    "customer": "CRG Automation",
    "contact": "",
    "address": "1700 Watterson Trail, Louisville, KY 40299",
  ...
  {
    "id": "sp-numina-po26-00450",
    "poNumber": "PO26-00450",
    "customer": "Numina Group, Inc.",
    "contact": "Yusuf Ertas",
    "address": "11480 Lakefield Drive, Johns Creek, GA 30097 (ships direct to Boston Scientific Corporation)",
  ...
  {
    "id": "sp-ete-gb00004043",
    "poNumber": "GB00004043",
    "customer": "Engine & Transmission Exchange",
    "contact": "",
    "address": "8700 North Servite Drive, Milwaukee, WI 53223 (8700 Warehouse - Manufacturing / Servite)",
...
function orderLineTotal(order) {
  return order.parts.reduce((sum, part) => sum + (Number(part.qty) || 0) * (Number(part.unitPrice) || 0), 0);
}
function sparePartsProgress(order) {
  const total = order.parts.length;
  const done = order.parts.filter(p => p.checked).length;
  return { total, done };
}
function countSpareParts() {
  let open = 0, done = 0, rushed = 0;
  spareParts.forEach(o => {
    if (o.rushed) rushed++;
    o.parts.forEach(p => { p.checked ? done++ : open++; });
  });
  return { open, done, orders: spareParts.length, rushed };
}
let activeSparePartsFilter = 'all'; // 'all' | 'rushed'
function renderSpareParts() {
  const container = document.getElementById('spareparts-container');
  if (!container) return;
  container.innerHTML = '';
  const today = toDateStr(new Date());
  const filtered = activeSparePartsFilter === 'rushed'
    ? spareParts.filter(o => o.rushed)
    : spareParts;
  if (filtered.length === 0) {
    container.innerHTML = '<div class="tasks-empty">No spare parts orders match this filter.</div>';
    updateSparePartsStats();
    return;
  }
  filtered.forEach((order) => {
    const orderIdx = spareParts.indexOf(order);
    const { total, done } = sparePartsProgress(order);
    const pct = total > 0 ? Math.round((done / total) * 100) : 0;
    const isOrderComplete = total > 0 && done === total;
    const isOverdue = order.dueDate && order.dueDate < today && !isOrderComplete;
    const block = document.createElement('div');
    block.className = 'spb-block' + (order.rushed ? ' spb-rushed' : '') + (isOrderComplete ? ' spb-complete' : '');
    const header = document.createElement('div');
    header.className = 'spb-header';
    header.innerHTML =
      '<div class="spb-header-left">' +
        '<div>' +
          '<div class="spb-title">' + order.customer + ' <span class="spb-po">PO #' + order.poNumber + '</span>' + (order.rushed ? ' <span class="spb-badge-rushed">RUSHED</span>' : '') + '</div>' +
          '<div class="spb-meta">' +
            (order.contact ? '<span>Attn: ' + order.contact + '</span>' : '') +
            '<span>' + order.address + '</span>' +
            '<span>Terms: ' + (order.terms || '—') + '</span>' +
            '<span' + (isOverdue ? ' class="spb-overdue"' : '') + '>Due ' + formatDisplayDate(order.dueDate) + '</span>' +
          '</div>' +
        '</div>' +
      '</div>' +
      '<div class="spb-header-right">' +
        '<div class="spb-total">' + formatMoney(order.total) + '</div>' +
        '<div class="spb-progress">' +
          '<div class="spb-progress-bar"><div class="spb-progress-fill" style="width:' + pct + '%;"></div></div>' +
          '<span class="spb-progress-text">' + done + ' / ' + total + '</span>' +
        '</div>' +
        '<span class="spb-toggle open">▲</span>' +
      '</div>';
    block.appendChild(header);
    const partsList = document.createElement('div');
    partsList.className = 'spb-parts';
    order.parts.forEach((part, partIdx) => {
      const row = document.createElement('div');
      row.className = 'part-row';
      const check = document.createElement('div');
      check.className = 'task-check' + (part.checked ? ' checked' : '');
      check.addEventListener('click', () => {
        spareParts[orderIdx].parts[partIdx].checked = !spareParts[orderIdx].parts[partIdx].checked;
        renderSpareParts();
      });
      const body = document.createElement('div');
      body.className = 'part-body';
      const lineTotal = (Number(part.qty) || 0) * (Number(part.unitPrice) || 0);
      body.innerHTML =
        '<div class="part-title' + (part.checked ? ' done' : '') + '">' + part.desc + (part.partNum ? ' <span class="part-num">#' + part.partNum + '</span>' : '') + '</div>' +
        '<div class="part-meta">' +
          '<span class="part-qty">Qty ' + part.qty + '</span>' +
          '<span class="part-price">' + formatMoney(part.unitPrice) + ' ea</span>' +
          '<span class="part-line-total">' + formatMoney(lineTotal) + '</span>' +
        '</div>';
      row.appendChild(check);
      row.appendChild(body);
      partsList.appendChild(row);
    });
    block.appendChild(partsList);
    header.addEventListener('click', () => {
      const tgl = header.querySelector('.spb-toggle');
      partsList.classList.toggle('collapsed');
      tgl.classList.toggle('open');
    });
    container.appendChild(block);
  });
...
function updateSparePartsStats() {
  const { open, done, orders, rushed } = countSpareParts();
  const elOpen = document.getElementById('sp-stat-open');
  const elDone = document.getElementById('sp-stat-done');
  const elOrders = document.getElementById('sp-stat-orders');
  const elRushed = document.getElementById('sp-stat-rushed');
  if (elOpen) elOpen.textContent = open;
  if (elDone) elDone.textContent = done;
  if (elOrders) elOrders.textContent = orders;
  if (elRushed) elRushed.textContent = rushed;
}
function wireSparePartsFilters() {
  const btns = document.querySelectorAll('.sp-filter-btn');
  if (!btns.length) return;
  btns.forEach(btn => {
    btn.addEventListener('click', () => {
      document.querySelectorAll('.sp-filter-btn').forEach(b => b.classList.remove('active'));
      btn.classList.add('active');
      activeSparePartsFilter = btn.dataset.filter;
      renderSpareParts();
    });
  });
}
...
const SPAREPARTS_FIELDS = [
  { key: 'poNumber', label: 'PO Number' },
  { key: 'customer', label: 'Customer' },
  { key: 'contact', label: 'Contact' },
  { key: 'address', label: 'Ship-To Address' },
  { key: 'orderDate', label: 'Order Date (YYYY-MM-DD)' },
  { key: 'dueDate', label: 'Due / Ship Date (YYYY-MM-DD)' },
  { key: 'terms', label: 'Payment Terms' },
  { key: 'total', label: 'Order Total ($)' },
  { key: 'notes', label: 'Notes' }
];
let editorTab = 'projects'; // 'projects' | 'tasks' | 'spareparts'
function updateEditorFooterButtons() {
  const addProjectBtn = document.getElementById('add-project-btn');
  const addSparePartsBtn = document.getElementById('add-spareparts-btn');
  if (addProjectBtn) addProjectBtn.style.display = editorTab === 'projects' ? '' : 'none';
  if (addSparePartsBtn) addSparePartsBtn.style.display = editorTab === 'spareparts' ? '' : 'none';
}
function injectEditorUI() {
    ...
    '<div id="editor-panel-inner">' +
      '<div id="editor-panel-header">' +
        '<div id="editor-tabs">' +
          ...
          '<button type="button" class="editor-tab-btn" data-tab="spareparts">Spare Parts</button>' +
        '</div>' +
      ...
      '<div id="editor-panel-footer">' +
        ...
        '<button type="button" id="add-spareparts-btn" style="display:none;">+ Add Spare Parts Order</button>' +
  ...
  panel.querySelectorAll('.editor-tab-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      updateEditorFooterButtons();
      renderEditorBody();
    });
  });
  document.getElementById('add-project-btn').addEventListener('click', addNewProject);
  document.getElementById('add-spareparts-btn').addEventListener('click', addNewSparePartsOrder);
...
function renderEditorBody() {
  ...
  if (editorTab === 'projects') {
    ...
    // Spare Parts tab
    spareParts.forEach((o, oIdx) => {
      const row = document.createElement('div');
      row.className = 'editor-row';
      let fieldsHtml = SPAREPARTS_FIELDS.map(f => {
        const val = (o[f.key] !== null && o[f.key] !== undefined ? o[f.key] : '').toString().replace(/"/g, '&quot;');
        return '<label class="editor-field">' + f.label + '<input type="text" data-oidx="' + oIdx + '" data-key="' + f.key + '" value="' + val + '"></label>';
      }).join('');
      let partRows = o.parts.map((part, partIdx) => {
        return (
          '<div class="editor-task-row">' +
            '<input type="checkbox" data-oidx="' + oIdx + '" data-pidx="' + partIdx + '" data-key="checked" ' + (part.checked ? 'checked' : '') + '>' +
            '<input type="text" class="editor-task-title" placeholder="Description" data-oidx="' + oIdx + '" data-pidx="' + partIdx + '" data-key="desc" value="' + (part.desc || '').replace(/"/g, '&quot;') + '">' +
            '<input type="text" class="editor-part-num" placeholder="Part #" data-oidx="' + oIdx + '" data-pidx="' + partIdx + '" data-key="partNum" value="' + (part.partNum || '').replace(/"/g, '&quot;') + '">' +
            '<input type="text" class="editor-part-qty" placeholder="Qty" data-oidx="' + oIdx + '" data-pidx="' + partIdx + '" data-key="qty" value="' + (part.qty != null ? part.qty : '') + '">' +
            '<input type="text" class="editor-part-price" placeholder="Unit $" data-oidx="' + oIdx + '" data-pidx="' + partIdx + '" data-key="unitPrice" value="' + (part.unitPrice != null ? part.unitPrice : '') + '">' +
            '<button type="button" class="editor-task-delete editor-part-delete" data-oidx="' + oIdx + '" data-pidx="' + partIdx + '">✕</button>' +
          '</div>'
        );
      }).join('');
      row.innerHTML =
        '<div class="editor-row-header">' +
          '<strong>' + (o.customer || 'New Order') + ' — PO #' + (o.poNumber || '—') + '</strong>' +
          '<label class="editor-install"><input type="checkbox" class="editor-rushed-cb" data-oidx="' + oIdx + '" ' + (o.rushed ? 'checked' : '') + '> Rushed</label>' +
          '<button type="button" class="editor-delete-btn editor-order-delete" data-oidx="' + oIdx + '">Delete Order</button>' +
        '</div>' +
        '<div class="editor-fields">' + fieldsHtml + '</div>' +
        '<div class="editor-task-block" style="margin-top:0.8rem;">' +
          '<div class="editor-row-header"><strong style="font-size:0.78rem;color:var(--text-muted);">Parts</strong>' +
            '<button type="button" class="editor-add-task-btn editor-add-part-btn" data-oidx="' + oIdx + '">+ Add Part</button>' +
          '</div>' +
          '<div class="editor-task-list">' + partRows + '</div>' +
        '</div>';
      body.appendChild(row);
    });
    body.querySelectorAll('.editor-fields input[type="text"]').forEach(input => {
      input.addEventListener('input', e => {
        const oIdx = parseInt(e.target.getAttribute('data-oidx'), 10);
        const key = e.target.getAttribute('data-key');
        const val = e.target.value;
        spareParts[oIdx][key] = key === 'total' ? (parseFloat(val) || 0) : val;
        refreshAllRenders();
      });
    });
    body.querySelectorAll('.editor-rushed-cb').forEach(cb => {
      cb.addEventListener('change', e => {
        const oIdx = parseInt(e.target.getAttribute('data-oidx'), 10);
        spareParts[oIdx].rushed = e.target.checked;
        refreshAllRenders();
      });
    });
    body.querySelectorAll('.editor-order-delete').forEach(btn => {
      btn.addEventListener('click', e => {
        const oIdx = parseInt(e.target.getAttribute('data-oidx'), 10);
        if (!window.confirm('Delete PO #' + spareParts[oIdx].poNumber + ' and all its parts? Staged locally until you Save.')) return;
        spareParts.splice(oIdx, 1);
        renderEditorBody();
        refreshAllRenders();
      });
    });
    body.querySelectorAll('.editor-task-row input[type="checkbox"]').forEach(cb => {
      cb.addEventListener('change', e => {
        const oIdx = parseInt(e.target.getAttribute('data-oidx'), 10);
        const pIdx = parseInt(e.target.getAttribute('data-pidx'), 10);
        spareParts[oIdx].parts[pIdx].checked = e.target.checked;
        refreshAllRenders();
      });
    });
    body.querySelectorAll('.editor-task-title, .editor-part-num, .editor-part-qty, .editor-part-price').forEach(input => {
      input.addEventListener('input', e => {
        const oIdx = parseInt(e.target.getAttribute('data-oidx'), 10);
        const pIdx = parseInt(e.target.getAttribute('data-pidx'), 10);
        const key = e.target.getAttribute('data-key');
        let val = e.target.value;
        if (key === 'qty' || key === 'unitPrice') val = parseFloat(val) || 0;
        spareParts[oIdx].parts[pIdx][key] = val;
        refreshAllRenders();
      });
    });
    body.querySelectorAll('.editor-part-delete').forEach(btn => {
      btn.addEventListener('click', e => {
        const oIdx = parseInt(e.target.getAttribute('data-oidx'), 10);
        const pIdx = parseInt(e.target.getAttribute('data-pidx'), 10);
        spareParts[oIdx].parts.splice(pIdx, 1);
        renderEditorBody();
        refreshAllRenders();
      });
    });
    body.querySelectorAll('.editor-add-part-btn').forEach(btn => {
      btn.addEventListener('click', e => {
        const oIdx = parseInt(e.target.getAttribute('data-oidx'), 10);
        spareParts[oIdx].parts.push({ id: 'sp-new-' + Date.now(), desc: 'New part', partNum: '', qty: 1, unitPrice: 0, checked: false });
        renderEditorBody();
        refreshAllRenders();
      });
    });
...
function addNewSparePartsOrder() {
  spareParts.push({
    id: 'sp-new-' + Date.now(),
    poNumber: '',
...
function refreshAllRenders() {
  ...
  if (document.getElementById('spareparts-container')) renderSpareParts();
}
async function saveToGitHub() {
  ...
  try {
    ...
    const sparePartsRegex = /\/\/ ===SPAREPARTS_START===[\s\S]*?\/\/ ===SPAREPARTS_END===/;
    if (!projectsRegex.test(currentContent) || !sparePartsRegex.test(currentContent)) {
      throw new Error('Could not find the required ===PROJECTS_START/END=== or ===SPAREPARTS_START/END=== markers in main.js — aborted to avoid corrupting the file.');
    }
    const newProjectsBlock = '// ===PROJECTS_START===\nconst projects = ' + JSON.stringify(projects, null, 2) + ';\n// ===PROJECTS_END===';
    const newSparePartsBlock = '// ===SPAREPARTS_START===\nconst spareParts = ' + JSON.stringify(spareParts, null, 2) + ';\n// ===SPAREPARTS_END===';
    let newContent = currentContent.replace(projectsRegex, newProjectsBlock);
    newContent = newContent.replace(sparePartsRegex, newSparePartsBlock);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Spare Parts page
  if (document.getElementById('spareparts-container')) {
    renderSpareParts();
    wireSparePartsFilters();
  }
  // Editor (all pages)
  injectEditorUI();
});
